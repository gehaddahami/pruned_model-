...
#%% pruning step 
#pruning function  
def prune_weights_l2(model, amount=0.3):
    total_params = 0
    pruned_params = 0
    masks = {}
    
    for name, module in model.named_modules():
        if isinstance(module, qnn.QuantConv1d) or isinstance(module, qnn.QuantLinear):
            module_name = name.split('.')
            last_name = module_name[-1]
            
            if last_name != '1' and last_name != '36':  # Exclude first and last layer
                weight_params = np.prod(module.weight.shape)
                total_params += weight_params
                print(f"Layer name: {name}, Weight shape: {module.weight.shape}, Total parameters: {total_params}")

                # Prune the weights based on L2 norm
                prune.ln_structured(module, name='weight', amount=amount, n=2, dim=0)
                
                # Create a mask 
                masks[name] = torch.ones_like(module.weight)
                masks[name][module.weight == 0] = 0  # Set pruned weights to 0 in the mask

                # Apply the mask to the pruned weights
                module.weight.data *= masks[name]

                # Count pruned parameters
                pruned_params += weight_params - np.count_nonzero(module.weight.detach().cpu().numpy())

    print(f"Total parameters: {total_params}")
    print(f"Pruned parameters: {pruned_params}")
    print(f"Sparsity percentage: {(pruned_params / total_params) * 100:.2f}%")
...
masks =prune_weights_l2(model, amount=amount)
# %% Loops for training and testing
# Training and testing loops  

def train_loop(model, train_loader, optimizer, criterion, sparsity_threshold, masks):
    losses = []
    model.train()
        ...
        for name, module in model.named_modules(): 
            if isinstance(module, qnn.QuantConv1d) or isinstance(module, qnn.QuantLinear): 
                if hasattr(module.weight, 'mask'):
                    module_weight_sparsity = torch.sum(module.weight == 0).item() / module.weight.numel()
                    module_weight_grad_sparsity = torch.sum(module.weight.grad == 0).item() / module.weight.grad.numel()

                    if module_weight_sparsity <= sparsity_threshold:
                        print(f"Layer: {name}, Weight Sparsity: {module_weight_sparsity:.4f}, Grad Sparsity: {module_weight_grad_sparsity:.4f}")

                        module.weight.grad *= module.weight.mask 
...
for epoch in range(num_epochs):
    ...
    loss_epoch = train_loop(model, train_data, optimizer, criterion, 0.20, masks=masks )
